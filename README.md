...
> **统一入口**: 不知道用哪个 Skill？直接说 **"帮我选 skill"**，`skill-orchestrator` 会帮你匹配最佳 Skill 或工作流组合。
>
> **当前规模**: 111 个 Skills，覆盖开发、设计、DevOps、测试、安全、记忆、工作、生活、学习、创意等场景。
...
├── skill-orchestrator/ # Skill编排器（111个Skill的统一入口，智能匹配+工作流推荐）
...
| 来源项目 | 贡献的Skill理念 |
|---------|----------------|
| **obra/superpowers** | systematic-debugging, test-driven-development, brainstorming, writing-plans |
| **Anthropic官方Skills** | frontend-design, skill-creator, web-artifacts-builder, mcp-builder, algorithmic-art 等 |
| **skills.sh find-skills** | skill-lookup (元技能理念) |
| **Top 20 Claude Code** | confidence-check, create-pr (自动化理念) |
| **forrestchang/andrej-karpathy-skills** | karpathy-skills (LLM编程避坑) |
| **mattpocock/skills** | opinionated-engineer, dictionary-of-ai-coding (工程化与术语) |
| **alchaincyf/huashu-design** | huashu-design (HTML原生设计) |
| **lewislulu/html-ppt-skill** | html-ppt-skill (HTML PPT生成) |
| **cathrynlavery/diagram-design** | diagram-design (编辑级图表) |
| **mukul975/cybersecurity-skills** | cybersecurity-skills (网络安全分析师技能库) |
| **containers/kubernetes-mcp-server** | DevOps/Kubernetes 工具链理念 |
| **hashicorp/terraform-mcp-server** | Terraform / IaC 理念 |
| **crystaldba/postgres-mcp** | PostgreSQL 进阶诊断理念 |
...
> 不知道用哪个 Skill？直接说： **"帮我选 skill"**、"推荐个 skill"、"完整工作流"  
> `skill-orchestrator` 会根据你的描述自动匹配最佳 Skill 或 Skill 组合。
...
| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **skill-orchestrator** | ⭐ Skill编排器：111个Skill的统一智能入口，场景匹配+工作流推荐 | 自建 |
| **skill-lookup** | Skills发现与安装助手，支持关键词/分类/场景搜索 | skills.sh |
| **skill-creator** | Skill创造者：元技能，创建/测试/优化Skills，含Eval框架 | Anthropic 官方 |
| **skill-quality-analyzer** | Skill质量评估元技能：结构/文档/安全/可维护性/触发精度五维评分 | Anthropic PR |

### 🤖 AI 基础设施与 Agent（8个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **mcp-builder** | 🆕 MCP构建指南：Model Context Protocol / Tool / Resource / Prompt 封装 | Anthropic官方+社区 |
| **multi-agent-orchestration** | 多Agent编排：并行执行，worktree隔离，AI团队协作框架 | 社区热门 |
| **agent-governance** | AI Agent治理框架：策略执行/信任评分/审计追踪/威胁检测 | Anthropic Issue |
| **ai-collaboration-safety** | AI协作安全指南：防幻觉/防逼疯/原子输出/熔断机制 | 自建 |
| **vibe-coding** | 🆕 Vibe Coding实战：自然语言驱动开发/护栏/重构固化 | 社区热门 |
| **repo-intelligence** | 🆕 仓库智能：代码库问答/变更影响分析/架构意图还原 | 自建 |
| **codebase-inventory-audit** | 代码库清单审计：孤儿代码/重复逻辑/文档缺口/技术债务量化 | Anthropic Issue |
| **dictionary-of-ai-coding** | AI编码术语词典：Skill/Prompt/Agent/Vibe Coding大白话解释 | mattpocock |

### 🧠 开发方法论与工作流（13个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **bmad-method** | AI驱动敏捷开发：一人顶一团队，多角色协作 | bmad-code-org |
...
| **openspec-sdd** | 规范驱动开发：OpenSpec六阶段+SDD+Gherkin验收场景 | OpenSpec |
| **dev-flow** | 智能开发工作流：自动感知六阶段，方法论自动编排 | 蒸馏（5合1） |
| **enterprise-iteration-agent** | 企业级全栈迭代工作流Agent：7阶段闭环/状态机/子Skill编排 | 自建 |
| **writing-plans** | 详细实施计划生成 | obra/superpowers |
| **brainstorming** | 结构化头脑风暴，coding前思考 | obra/superpowers |
| **systematic-debugging** | 系统化调试方法论，4阶段根因分析 | obra/superpowers |
| **karpathy-skills** | LLM编程避坑：十大戒律/调试流程/提问模板 | karpathy-skills |
| **opinionated-engineer** | 工程化脚手架：强制TDD/类型安全/错误处理/日志规范 | mattpocock/skills |
| **caveman-skill** | 洞穴人Token优化：削减75%输出token，压缩记忆文件 | juliusbrussee/caveman |
| **confidence-check** | AI自我置信度评估 | Top 20 Skills |
...
| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **frontend-design** | 前端设计规范：拒绝AI slop，6大视觉方向 | Anthropic 官方 |
| **frontend-code-review** | 前端代码结构化审查：React/TS/TSX性能/可维护性/可访问性 | 社区热门 |
| **web-artifacts-builder** | 复杂Web构件构建器：React+Tailwind+shadcn/ui | Anthropic官方 |
| **browser-use** | 浏览器自动化：截图/填表/数据抓取/Playwright | awesome-agent-skills |
| **stop-slop** | 去除AI味：识别并清除AI填充语/套话/slop模式 | 社区热门 |

### 🗄️ 后端与数据库（6个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **api-doc-generator** | API文档生成：Swagger注解/OpenAPI文档 | awesome-agent-skills |
| **database-designer** | 数据库设计：ERD/Mermaid + MySQL DDL + Java Entity | superno/claude-skills-sup |
| **erd-document** | 完整系统设计文档：Word/PDF（11章节） | 自建 |
| **postgres-pro** | 🆕 PostgreSQL进阶诊断：慢查询/EXPLAIN/索引/连接池/VACUUM/锁分析 | GitHub热门 |
| **backend-change-flow** | 后端需求驱动变更工作流：读代码→析需求→影响分析→编码→Review | 自建 |
| **enterprise-crm-fullstack** | 企业级CRM全栈开发规范：Vue 2+Element UI+Java | 自建 |

### 🐳 DevOps 与云（2个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **devops-toolchain** | 🆕 DevOps工具链：Docker/Kubernetes/Terraform/GitHub Actions/CI/CD | GitHub热门 |
| **cloudflare-worker** | Cloudflare Worker边缘函数：Wrangler/TypeScript/KV/R2/D1 | Anthropic官方+社区 |

### 🧪 测试与质量（6个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **test-driven-development** | TDD测试驱动开发，红-绿-重构循环 | obra/superpowers |
| **testing-patterns** | 全栈测试模式：单元/React组件/E2E/Mock，测试金字塔 | Anthropic PR |
| **webapp-testing** | Web应用自动化测试：Playwright侦察→行动 | 社区实践 |
| **quality-gate** | 代码质量门控：提交前五维检查 | 蒸馏（6合1） |
| **create-pr** | 自动生成规范PR：分析git diff | 社区热门 |
| **document-typography** | 文档排版质量控制：孤行寡段/编号对齐/字体层级 | Anthropic PR |

### 🔒 安全（2个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **security-audit** | 代码安全审计：CodeQL/Semgrep，OWASP Top 10 | trailofbits/skills |
| **cybersecurity-skills** | 网络安全分析师技能库：754技能/26领域/MITRE ATT&CK | mukul975 |

### 🧠 记忆与经验（7个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **memory-hub** | 统一记忆中枢：自动路由四层记忆，告别选择困难 | 蒸馏（6合1） |
| **beads-memory** | 编码Agent记忆升级：跨会话持久化代码库上下文 | gastownhall/beads |
| **claude-mem** | 跨会话持久记忆：自动捕获/AI压缩/向量检索 | thedotmack/claude-mem |
| **supermemory** | 跨会话持久记忆：自动加载上下文，向量检索 | SuperMemory |
| **memory-bank** | 项目记忆系统：AGENTS/INDEX/模板/ADR | 自建 |
| **memory-system** | 记忆系统：Redis缓存/多级缓存/分布式Session | 自建 |
| **hermes-experience** | 经验沉淀：patterns/snippets/rules | hermes-agent |

### 🛠️ 开发工具与效率（7个）

| Skill名称 | 功能 | 来源 |
|----------|------|------|
| **git-commit** | 规范化Git提交：Conventional Commits | awesome-agent-skills |
| **solo-parallel-dev** | 单人多分支并行开发：worktree隔离/分支看板 | 自建 |
| **cron-expression** | Cron表达式：自然语言生成/Java+MySQL实现 | 自建 |
| **skill-accelerator** | 技能加速器·滑翔伞学习法：六阶段通用方法论 | 自建 |
| **dev-toolkit-integrator** | 禅道·Jira·Wiki三工具集成 | 自建 |
| **claude-obsidian-reporter** | Obsidian报告生成：Git提交转日/周/月报 | Anthropic PR |
| **hads** | 人-AI双读文档标准：Markdown语义标记 | Anthropic PR |

### 💼 职场工作（7个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **anti-pua** | 职场反PUA话术 | "老板说"、"怎么怼"、"被PUA了" |
| **meeting-notes** | 整理会议纪要 | "整理会议纪要"、"会议记录太乱" |
| **weekly-report** | 生成专业周报 | "写周报"、"本周工作总结" |
| **leave-request** | 请假话术 | "怎么请假"、"请假理由" |
| **elevator-pitch** | 电梯演讲 | "电梯演讲"、"一句话介绍" |
| **argue-winner** | 应对投诉/甩锅 | "吵架"、"怎么回击"、"被甩锅" |
| **bs-translator** | 废话翻译 | "废话"、"听不懂人话" |

### 🎨 生活娱乐（8个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **food-picker** | 纠结吃什么 | "吃什么"、"点外卖选择" |
| **gift-advisor** | 礼物参谋 | "送什么礼物"、"生日礼物" |
| **outfit-weather** | 穿搭天气 | "明天穿什么"、"穿搭建议" |
| **recipe-random** | 菜谱随机 | "做什么菜"、"菜谱随机" |
| **travel-planner** | 旅行规划 | "旅行计划"、"旅游攻略" |
| **weather-pro** | 专业气象分析 | "分析天气"、"气象数据" |
| **music-recommender** | 音乐推荐与歌单 | "推荐首歌"、"适合跑步的音乐" |
| **personality-test** | 人格测试大全 | "MBTI"、"人格测试"、"星座性格" |

### 📚 学习成长（5个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **wrong-answer** | 错题分析 | "错题分析"、"帮我讲题" |
| **knowledge-card** | 知识卡片 | "知识卡片"、"帮我背书" |
| **study-buddy** | 陪孩子学习 | "陪孩子学习"、"听写" |
| **jargon-translator** | 术语翻译 | "术语翻译"、"听不懂" |
| **ai-trend-radar** | AI趋势雷达 | "AI趋势"、"AI热词" |

### 🏥 健康科普（2个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **cardiac-arrest-guide** | 心脏骤停科普与急救 | "心脏骤停"、"心肺复苏"、"AED" |
| **mental-health-check** | 专业心理健康自评 | "焦虑测试"、"心理测试"、"burnout" |

### 🎮 传统文化游戏（2个）

| Skill名称 | 功能 | 来源 | 触发关键词 |
|----------|------|------|-----------|
| **idiom-chain** | 成语接龙 | 原创 | "成语接龙"、"玩接龙" |
| **feihua-ling** | 古诗词飞花令 | 原创 | "飞花令"、"诗词接龙" |

### 🎨 视觉内容设计（6个）

| Skill名称 | 功能 | 来源 | 触发关键词 |
|----------|------|------|-----------|
| **html-ppt-skill** | HTML PPT生成 | lewislulu/html-ppt-skill | "做PPT"、"生成演示文稿" |
| **diagram-design** | 编辑级图表设计 | cathrynlavery/diagram-design | "画架构图"、"画流程图" |
| **huashu-design** | 花叔HTML原生设计 | alchaincyf/huashu-design | "做网页"、"做落地页" |
| **taste-skill** | 反平庸UI设计 | Leonxlnx | "反平庸设计"、"去AI味UI" |
| **remotion-video** | Remotion React视频生成 | Remotion官方 | "生成视频"、"React视频" |
| **algorithmic-art** | 算法艺术生成 | Anthropic官方 | "生成艺术"、"p5.js艺术" |

### 💡 创意写作（10个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **poetry-scenery** | 诗意风景描述 | "拍了xx照片"、"发朋友圈" |
| **moments-copywriter** | 朋友圈/小红书文案 | "朋友圈文案"、"发圈词" |
| **letter-future** | 给未来写封信 | "给未来写封信"、"时间胶囊" |
| **epitaph-generator** | 墓志铭/人生思考 | "墓志铭"、"人生感悟" |
| **horoscope** | 星座运势 | "星座"、"抽签"、"运势" |
| **emoji-translator** | emoji解读 | "emoji翻译" |
| **qiaopi-style** | 侨批体书信生成 | "转成侨批体"、"侨批" |
| **rural-story-writer** | 乡土叙事小说 | "写乡土小说" |
| **sleep-story** | 睡前故事 | "睡前故事"、"睡不着" |
| **year-summary** | 年度总结 | "年终总结"、"年度复盘" |

### 🧠 思考决策（3个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **relationship-patterns** | 亲密关系模式识别 | "关系模式"、"是不是PUA" |
| **choice-helper** | 两难选择 | "选择困难"、"帮我选" |
| **venting-hole** | 吐槽树洞 | "吐槽"、"心情不好" |

### 🏠 居家生活（5个）

| Skill名称 | 功能 | 触发关键词 |
|----------|------|-----------|
| **photo-organizer** | 照片整理 | "整理照片"、"相册太乱" |
| **declutter-judge** | 断舍离裁判 | "断舍离"、"扔不扔" |
| **home-design** | 装修灵感 | "装修灵感"、"北欧风" |
| **plant-care** | 绿植养护 | "绿植养护" |
| **home-storage** | 家居收纳 | "收纳"、"整理房间" |
...
| 2026-04-29 | 新增 | skill-orchestrator（Skill编排器·统一入口） | 自建（111个Skill智能匹配+标准工作流推荐） |
...
| 2026-06-04 | 新增 | enterprise-iteration-agent（企业级全栈迭代工作流Agent·7阶段闭环/状态机/路径感知/子Skill编排） | 自建（串联111个Skill形成开发闭环） |
...
| 2026-08-27 | 🔄重构 | README 分类体系重构：将 111 个 Skill 重新划分为 18 个主题类别，突出 skill-orchestrator 统一入口 | 优化 Skill 发现与使用体验 |
